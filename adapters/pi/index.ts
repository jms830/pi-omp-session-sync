...
function buildProvenance(source: DiscoveredSession, canonicalId: string, messageCount: number, skippedUnknown: number): string {
	return [
		`Synced from OMP session ${source.id}`,
...
async function importOne(ctx: CommandContext, parsed: ParsedArgs, source: DiscoveredSession): Promise<{ imported: boolean; message?: string }> {
	const loaded = loadSessionFile(source.file);
	const converted = convertOmpToPi(loaded);
	const targetDir = PiSessionManager.create(source.cwd).getSessionDir();
	...
	const result = await ctx.switchSession(filePath, {
		withSession: async (replacementCtx) => replacementCtx.ui.setEditorText(""),
	});
...
async function selectAndImport(ctx: CommandContext, parsed: ParsedArgs): Promise<void> {
	if (!ctx.hasUI) {
	...
	if (sessions.length === 0) {
		ctx.ui.notify(parsed.selector ? `No OMP sessions matched: ${parsed.selector}` : "No OMP sessions found", "warning");
		return;
	}
	const choices = sessions.map((session) => formatChoice(session, registry));
	const selected = await ctx.ui.select("Sync OMP session into Pi", choices);
	if (!selected) {
	...
	if (!session) {
		ctx.ui.notify("Selected OMP session could not be resolved", "error");
		return;
	}
	const result = await importOne(ctx, parsed, session);
...
function handleList(ctx: CommandContext, parsed: ParsedArgs): void {
	const registry = loadImportRegistry(parsed.registryPath);
	const sessions = listSessions(parsed.sourceDir, { cwd: parsed.cwd, search: parsed.selector, since: parsed.since, updatedSince: parsed.updatedSince, limit: parsed.limit });
...
async function handleOpen(ctx: CommandContext, parsed: ParsedArgs, sessionId: string | undefined): Promise<void> {
	if (!sessionId) {
		ctx.ui.notify("Usage: /pi-omp-sync open <omp-session-id>", "error");
		return;
	}
	const entry = getSyncedSession(loadImportRegistry(parsed.registryPath), SOURCE, RUNTIME, sessionId);
	if (!entry) {
		ctx.ui.notify(`No Pi session synced from OMP ${sessionId}`, "warning");
		return;
	}
	const result = await ctx.switchSession(entry.targetSessionFile, {
		withSession: async (replacementCtx) => replacementCtx.ui.notify(`Opened Pi session synced from OMP ${sessionId}`, "info"),
	});
...
export default function piOmpSyncExtension(pi: ExtensionAPI): void {
	pi.registerCommand("pi-omp-sync", {
		description: "Sync OMP sessions into native Pi sessions",
		getArgumentCompletions: () => [
			{ value: "list ", label: "list" },
			{ value: "status", label: "status" },
			{ value: "all --dry-run", label: "all --dry-run" },
			{ value: "all", label: "all" },
			{ value: "open ", label: "open <omp-session-id>" },
		...
		handler: async (args, ctx) => {
			try {
				...
				if (selector && isLikelySessionId(selector)) {
					...
					if (!source) {
						ctx.ui.notify(`OMP session not found: ${selector}`, "warning");
						return;
					}
